...
const finalists=Object.entries(out.phaseAWinners??{}).map(([coverage,w])=>({coverage:Number(coverage),config:w?.config??null,phaseARoi:w?.phaseA?.roi??null,phaseANetPnlEUR:w?.phaseA?.netPnlEUR??null}));
const economicPromotionGate={amendmentVersion:amendment.version,scope:amendment.scope,createdAt:amendment.createdAt,primaryStatisticalHypothesisUnchanged:true,phaseBOutcomeDisclosedAtAmendment:false,candidateSelectionUnchanged:true,finalists,requirements:{originalStatisticalConfirmationPass:true,positivePhaseBRoi:true,positivePhaseBNetPnlEUR:true,allRequired:true},pass:false,reason:out.gates?.fixedFinalComplete===true?'NO_FINALIST_PASSED_ECONOMIC_PROMOTION_GATE':'PHASE_B_STILL_BLINDED',confirmation:null};
if(out.gates?.fixedFinalComplete===true){
  const confirmation={};let anyEconomicPass=false;
  for(const [coverage,c] of Object.entries(out.confirmation??{})){
    const statisticalPass=c?.pass===true,positiveRoiPass=Number(c?.phaseB?.roi)>0,positiveNetPnlPass=Number(c?.phaseB?.netPnlEUR)>0,economicPass=statisticalPass&&positiveRoiPass&&positiveNetPnlPass;
    confirmation[coverage]={statisticalPass,positiveRoiPass,positiveNetPnlPass,economicPass};if(economicPass)anyEconomicPass=true;
  }
