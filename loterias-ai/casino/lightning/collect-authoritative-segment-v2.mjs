...
const res=await fetch(ENDPOINT,{headers:{accept:'application/json','user-agent':'LoteriasAI-segment-v2/1.0','cache-control':'no-cache, no-store, max-age=0',pragma:'no-cache'}});
...
const bridgeSafe=!latestExisting||!firstFresh||Boolean(bridgeSeconds>=0&&bridgeSeconds<=maxBridge);
let persistedRows=0;
...
const status={
  ...
  continuity:{previousLastAt:latestExisting?.timestamp??null,firstFreshAt:firstFresh?.timestamp??null,bridgeSeconds,maximumBridgeSeconds:maxBridge,bridgeSafe,maxObservedInterRoundSeconds:maxGap},
  ...
  guards:{strictlyPostBarrier:true,oldAmbiguousGapImported:false,alternateSourceUsed:false,automaticBettingAllowed:false,realMoneyAllowed:false,realStakeEUR:0}
};
