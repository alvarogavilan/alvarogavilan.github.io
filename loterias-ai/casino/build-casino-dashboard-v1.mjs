...
const catalog=read(CAT)||{},spec=read(SPEC)||{},br=read(BREAK)||{},ev=read(EV)||{},hot=read(HOT)||{},jpk=read(JPK)||{},aog=read(AOG)||{},seg=read(SEG)||{},tim=read(TIM)||{},lag=read(LAG)||{},ready=read(READY)||{};
...
const payload={
  ...
  economics:{
    ...
    progressiveNetworks:{jackpotKing:ready.progressiveNetworks?.jackpotKing||null,ageOfGods:ready.progressiveNetworks?.ageOfGods||null,sharedPlayUZU:shared},
    mechanismRadar:{
