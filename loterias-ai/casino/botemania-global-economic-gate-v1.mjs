...
const validatedEconomicCandidates=executionLanes.filter(laneIsFullyExecutable).map(lane=>({
  slug:lane?.game?.id||lane?.id||'unknown',laneId:lane?.id||null,type:lane?.type||null,name:lane?.game?.name||null,url:lane?.game?.url||null,
  source:'EDGE_LIVE_MULTI_EXECUTION_PLAN',status:'FULL_EXECUTION_GATE_PASSED',stakePerSpinEUR:Number(lane.order.stakePerSpinEUR),maxSpins:Number(lane.order.maxSpins),maxTotalStakeEUR:Number(lane.order.maxTotalStakeEUR),validUntil:lane.order.validUntil,
  observedAt:lane?.current?.observedAt||lane?.evidence?.observedAt||null,currentJackpotEUR:Number.isFinite(Number(lane?.current?.jackpotEUR))?Number(lane.current.jackpotEUR):null,breakEvenJackpotEUR:Number.isFinite(Number(lane?.economic?.breakEvenJackpotEUR))?Number(lane.economic.breakEvenJackpotEUR):null,
