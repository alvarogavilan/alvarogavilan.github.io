...
const VERSION='betfair-apmccoy-reviewed-race-bound-v1.1-feed-age-budget';
...
export function deriveBetfairApMcCoyReviewedRaceLowerBound({reviewedCycles,actionLatencyReview,raceAssumptionsReview,confidence=0.95}={}){
  ...
  return {
    ...
    reason:'CODE_REVIEWED_AP_MCCOY_CONSERVATIVE_RACE_LOWER_BOUND_AVAILABLE_WITH_FEED_AGE_BUDGET',
    ...
    bindingScopeKey:[...bindings][0],requestExecIntervalSeconds:exec,
    reviewedRaceLowerBoundAvailable:true,usableForRaceEvidence:true,usableForExecution:false,
    scientificUse:'Derives the AP McCoy first-bet race lower confidence bound from code-allowlisted prospective post-GHT survival reviews at a predeclared execution-safe window equal to the maximum permitted current feed age (two requestExecIntervals) plus separately reviewed manual-action latency. This prevents a subtle error where a bound at manual latency alone would ignore time already consumed between server detection and the decision. Latency remains independent of survival outcomes, all ambiguous outcomes are retained as failures, and complete-ledger/binomial/exchangeability assumptions require code-owned review. The result is race evidence only; exact served stake, current state and final decision-time exposure must still be checked.',
    execution:execution(),
    hardGuards:{onlineOnly:true,nonPromoOnly:true,reviewedCyclesOnly:true,codeOwnedSurvivalReviewAllowlist:true,codeOwnedLatencyReviewAllowlist:true,codeOwnedRaceAssumptionReviewAllowlist:true,latencyIndependentOfSurvivalOutcomes:true,maxDecisionFeedAgeIntervalsFrozen:true,raceWindowIncludesFeedAgePlusActionLatency:true,ambiguousCyclesCannotBeDropped:true,ambiguousCountedAsFailure:true,completeProspectiveLedgerRequired:true,sameBindingRequired:true,sameCadenceRequired:true,notExecutionAuthority:true,servedStakeStillRequired:true,currentFreshStateStillRequired:true,noWagerProbe:true,noAutomaticBetting:true,realMoneyAllowed:false}
  };
}
