import {isApprovedBet365FrankSurvivalReviewCommit} from './bet365-frank-post-ght-survival-review-v1.mjs';

const VERSION='bet365-frank-post-ght-survival-curve-v1';
const REVIEW_VERSION='bet365-frank-post-ght-survival-review-v1';
...
function bindingKey(v){const b=v?.bindingScope||{};return [String(b.jackpotsCasino||'').toLowerCase(),String(b.tickerEndpoint||''),String(b.instanceCode||'')].join('|');}
...
export function classifyBet365FrankReviewedSurvivalCycleAtLatency(reviewedCycle,latencySeconds){
...
export function deriveBet365FrankFrozenHorizonSurvivalCurve({reviewedCycles,confidence=0.95}={}){
  ...
  for(const x of list){
    ...
    if(!isApprovedBet365FrankSurvivalReviewCommit(x.reviewCommit))return fail('CYCLE_REVIEW_COMMIT_NOT_CODE_ALLOWLISTED',{cycleId:x.cycleId||null,reviewCommit:x.reviewCommit||null});
  ...
  const curve=HORIZON_INTERVALS.map(intervals=>{
    ...
    for(const x of list){const r=classifyBet365FrankReviewedSurvivalCycleAtLatency(x,latencySeconds);if(r.classification==='SUCCESS')successes++;else if(r.classification==='FAILURE')observedFailures++;else ambiguous++;classifications.push({cycleId:x.cycleId,classification:r.classification});}
    const conservativeFailures=n-successes;
    return {horizonIntervals:intervals,latencySeconds,totalReviewedCycles:n,strictSuccessCount:successes,observedFailureCount:observedFailures,ambiguousCount:ambiguous,conservativeFailureCount:conservativeFailures,ambiguousCountedAsFailureForBound:true,confidence:c,method:'ONE_SIDED_CLOPPER_PEARSON_BINOMIAL_CONSERVATIVE_AMBIGUOUS_AS_FAILURE',survivalProbabilityLowerBound:cpLower(n,successes,c),classifications};
  });
  return {
    version:VERSION,mode:'OFFLINE_REVIEWED_PROSPECTIVE_FRANK_POST_GHT_FROZEN_HORIZON_CURVE_NO_PLAY',valid:true,
    reason:'REVIEWED_FROZEN_HORIZON_SURVIVAL_CURVE_AVAILABLE_ACTION_LATENCY_AND_EXECUTION_SEMANTICS_STILL_UNFROZEN',
    operator:'bet365 Spain',market:'ES',target:{title:'Frank Bruno: Sporting Legends',gameCode:'gpas_slfbruno_pop'},
    ...
    actionLatencyFrozen:false,bet365FollowingDayRuleAdoptionVerified:false,servedTenCentJackpotEligibilityVerified:false,
    ...
    scientificUse:'Transforms only independently reviewed, code-allowlisted prospective Frank post-GHT survival cycles into a predeclared 1-to-12 requestExecInterval survival curve. No latency is selected from outcomes. For each horizon, only a confirmed unchanged/unreset state at or beyond the threshold counts as success; an observed award/reset strictly before the threshold counts as failure; every unresolved interval is retained and conservatively counted as failure in the Clopper-Pearson denominator. The curve remains descriptive research until an independent action-latency policy is frozen without using these outcomes, the exact bet365 following-day rule and €0.10 jackpot eligibility are reviewed, and the sampling/exchangeability assumptions needed for race probability are independently closed.',
    execution:execution(),
    hardGuards:{onlineOnly:true,nonPromoOnly:true,reviewedCodeAllowlistedCyclesOnly:true,frozenHorizonsOneThroughTwelve:true,latencyCannotBeSelectedFromCurve:true,ambiguousCyclesCannotBeDropped:true,ambiguousAreFailuresForLowerBound:true,sameBindingRequired:true,sameCadenceRequired:true,noPointEstimateExecution:true,operatorRuleStillRequired:true,tenCentEligibilityStillRequired:true,binomialAssumptionsStillRequired:true,noWagerProbe:true,noAutomaticBetting:true,realMoneyAllowed:false},
  };
}
