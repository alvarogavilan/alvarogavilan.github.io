...
for(const g of Array.isArray(census?.rows)?census.rows:[]){
  const base=Number(g?.baseRtpPct)/100,active=Number(g?.activeContributionPct)/100,conflict=g?.contributionConsistency?.mechanicsVsRtpConflict===true;
