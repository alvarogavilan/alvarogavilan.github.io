...
const census=read(CENSUS),flow=read(FLOW),alloc=read(ALLOC);
...
const out={version:'botemania-jpk-current-all-games-ev-screen-v1',generatedAt:new Date().toISOString(),operator:'botemania-es',observedAt:flow?.latest?.observedAt||null,potsEUR:pots,networkAllocationProspectivelyValidated:Boolean(net),networkAllocationShares:net,model:{purpose:'CURRENT_STATE_RESEARCH_SENSITIVITY_ONLY',hazardFamily:'BETA_ALPHA_1',alphaGrid,kingCredit:'ZERO_CONSERVATIVE',capHypothesisEUR:{ROYAL:3500,REGAL:35000},seedHypothesisEUR:{ROYAL:500,REGAL:5000},exactSpainMbwbKnown:false,exactHazardKnown:false,warning:'Cap and hazard are unverified. A screen result cannot authorize wagering.'},summary:{gamesScanned:rows.length,screenableGames:ranked.length,robustAtOrAbove100Count:robust.length,bestCurrentResearchCandidate:ranked[0]?{slug:ranked[0].slug,title:ranked[0].title,worstRtpPct:ranked[0].currentZeroKingSensitivity.worstRtpPct,bestRtpPct:ranked[0].currentZeroKingSensitivity.bestRtpPct,robustAtOrAbove100:ranked[0].currentZeroKingSensitivity.robustAtOrAbove100}:null,anyCurrentResearchScreenPass:robust.length>0},ranked,decision:{economicPromotionAllowed:false,realMoneyAllowed:false,automaticBettingAllowed:false,reason:robust.length?'RESEARCH_SCREEN_ONLY_EXACT_MBWB_AND_HAZARD_STILL_REQUIRED':'NO_SCREENABLE_BOTEMANIA_JPK_TITLE_ROBUSTLY_REACHES_100_CURRENTLY'},guards:{validatedNetworkAllocationUsedOnlyAtNetworkLevel:true,noPerTitleSplitClaim:true,noMainKingCredit:true,noCapHypothesisAsFact:true,noHazardHypothesisAsFact:true,noBetting:true,realMoneyAllowed:false}};
