...
const exactRoyal=Number(mbwb?.mustBeWonBeforeEUR?.ROYAL);
const exactRegal=Number(mbwb?.mustBeWonBeforeEUR?.REGAL);
...
const caps={
  ROYAL:{seed:500,cap:exactMbwb?exactRoyal:3500},
  REGAL:{seed:5000,cap:exactMbwb?exactRegal:35000}
};
...
const currentQ={
  ROYAL:clamp((currentRoyal-caps.ROYAL.seed)/(caps.ROYAL.cap-caps.ROYAL.seed)),
  REGAL:clamp((currentRegal-caps.REGAL.seed)/(caps.REGAL.cap-caps.REGAL.seed))
};
...
const out={
  version:'botemania-jpk-near-cap-ev-scenarios-v1.5-exact-spain-mbwb',generatedAt:new Date().toISOString(),operator:'botemania-es',purpose:'SENSITIVITY_SCREEN_ONLY_NOT_A_WAGER_RECOMMENDATION',
  inputs:{
    baseRtp:BASE_RTP,
    activeContributionShares:active,
    reserveShares:reserve,
    capHypothesisEUR:capValues,
    capEvidenceEUR:exactMbwb?capValues:null,
    capSource:exactMbwb?'BOTEMANIA_SPAIN_IN_GAME_OPERATOR_UI':'FALLBACK_RESEARCH_HYPOTHESIS',
    mbwbEvidenceFile:exactMbwb?MBWB:null,
    mbwbEvidenceImageSha256:exactMbwb?(mbwb?.image?.sha256||null):null,
    seedHypothesisEUR:{ROYAL:500,REGAL:5000},
    capHypothesisVerifiedInBotemania:exactMbwb,
    seedHypothesisVerifiedInBotemania:false,
    alphaGrid,
    stressExtensionReason:'Hazard remains unknown; alpha grid is sensitivity-only even after exact Spanish MBWB caps were recovered.'
  },
  model:{
    family:'HIDDEN_DROP_THRESHOLD_BETA_ALPHA_1_ON_SEED_TO_CAP',
    conditionalHazard:'activeContribution/span * alpha*q^(alpha-1)/(1-q^alpha)',
    warning:'Royal/Regal MBWB caps are exact from Botemania Spain in-game UI when evidence is present. Reserve seed and exact server-side hazard remain unknown; crossings are not proof of positive EV.',
    selectedGameContributionSplitAssumption:'Fishin Frenzy 2.32% active contribution is assumed to inherit the prospectively validated approximately 60/20/20 network allocation. Per-title hazard remains unverified.',
    oneTierConservativeDefinition:'A Royal-only or Regal-only threshold gives zero EV credit to the other guaranteed pot and zero credit to the main Jackpot King.'
  },
  current:{
    observedAt:flow?.latest?.observedAt||null,
    potsEUR:flow?.latest?.potsEUR||{},
    normalizedSeedToCap:currentQ,
    scenarios:currentScenarios,
    allModeledRtpBelowOne:{ZERO_CONSERVATIVE:conservativeAll.every(x=>x.totalRtp<1),ACTIVE_PLUS_RESERVE_STRUCTURAL:structuralAll.every(x=>x.totalRtp<1)},
    robustConservativeScreenPass,robustRoyalOnlyPass,robustRegalOnlyPass,
    worstConservativeRtp:worstConservativeRtp==null?null:+worstConservativeRtp.toFixed(6),
    bestConservativeRtp:bestConservativeRtp==null?null:+bestConservativeRtp.toFixed(6)
  },
  ...
  decision:{
    ...
    currentScreenPass:robustConservativeScreenPass,
    currentRoyalOnlyScreenPass:robustRoyalOnlyPass,
    currentRegalOnlyScreenPass:robustRegalOnlyPass,
    screenMeaning:'Exact Botemania Spain Royal/Regal MBWB caps are used when available, but seed and hazard remain sensitivity assumptions. A screen pass cannot authorize wagering.',
  ...
  guards:{
    forwardCrossingOnly:true,
    noSingleDistributionClaim:true,
    exactMbwbOnlyUsedWhenOperatorUiEvidencePresent:true,
    seedHypothesisNeverAsFact:true,
    noMainKingDoubleCount:true,
    oneTierRoutesGiveNoOtherJackpotCredit:true,
