...
export function parsePlaytechMhbTickerXml(xml,{nowEpochSeconds=Math.floor(Date.now()/1000)}={}) {
  const s=String(xml||'');
  const out=[];
  ...
  for(const gm of s.matchAll(gameRe)){
    ...
    const amountMatch=/<amount\b([^>]*)>([^<]*)<\/amount>/i.exec(gm[2]);
    ...
    out.push({
      ...
      amount,currency:aa.currency||null,sign:aa.sign||null,stepPerSecond:finite(aa.step),
  ...
  return {version:'playtech-mhb-ticker-parser-v1',rows:out,guards:{parserOnly:true,noBetting:true,realMoneyAllowed:false}};
}
