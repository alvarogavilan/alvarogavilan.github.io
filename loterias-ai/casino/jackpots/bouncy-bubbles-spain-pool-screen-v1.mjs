...
async function probeGraphql(name){
  ...
  for(const venture of ventures[name]){
    const headers={accept:'application/json','content-type':'application/json',origin:c.origin,referer:c.origin+'/','cache-control':'no-cache, no-store, max-age=0','user-agent':'edge-bouncy-spain-screen/1.2'};if(venture)headers.venture=venture;
...
async function pageFacts(name){
  const c=cfg[name],r=await fetchText(c.page,{headers:{accept:'text/html','user-agent':'edge-bouncy-spain-screen/1.2'}});const s=clean(r.text),has=x=>s.includes(x.toLowerCase());
...
async function tripleFacts(name){
  const c=cfg[name],r=await fetchText(c.triple,{headers:{accept:'text/html','user-agent':'edge-bouncy-spain-screen/1.2'}});const s=clean(r.text),has=x=>s.includes(x.toLowerCase());
...
const bouncyLinkedBoth=bt.facts.boteMayorSharedWithBouncy===true&&mt.facts.boteMayorSharedWithBouncy===true;
const resetTogetherBoth=bt.facts.allLinkedPotsResetTogether===true&&mt.facts.allLinkedPotsResetTogether===true;
const topologyConfirmedBoth=bouncyLinkedBoth&&resetTogetherBoth;
...
const out={
  version:'bouncy-bubbles-spain-pool-screen-v1.2-evidence-safe',observedAt,targetId:TARGET_ID,
  ...
  comparison:{
    ...
    note:bothTargets&&sameRules?'Both Spain operators expose the same exact feed id and matching public game rules. The requests are sequential, not atomic; a one-cent difference is compatible with meter movement and does not prove or disprove one instantaneous shared pool. Do not upgrade to exact shared-pool identity without synchronized evidence.':'Cross-operator identity family not established.'
  },
  ...
  sharedResetTopology:{
    ...
    bouncyLinkedRoleVerified:bouncyLinkedBoth,
    linkedPotsResetTogetherVerified:resetTogetherBoth,
    status:topologyConfirmedBoth?'VERIFIED_BOTH_OPERATOR_PAGES':'NOT_REPRODUCED_ON_CURRENT_PUBLIC_PAGES',
    priorClaimStatus:topologyConfirmedBoth?'REPRODUCED':'RETRACTED_UNTIL_REPRODUCED',
    implicationForInference:{
      ...
      reason:topologyConfirmedBoth?'Current public Spain pages reproduce the linked-reset topology, so a Burbujas meter drop cannot be attributed to a Burbujas-originating hit without identifying the triggering game/tier.':'Current public pages did not reproduce the previously asserted shared-reset topology. Therefore no topology-based hit attribution is allowed. Independently, exact feed-to-game identity and Burbujas-specific hit probability remain unresolved, so reset counts still cannot be treated as Burbujas pHit observations.'
    }
  },
  ...
  interpretation:{crossOperatorIdentityFamilyStronglySupported:Boolean(bothTargets&&sameRules),sameInstantaneousSpanishPoolVerified:false,exactBotemaniaFeedToGameIdentityVerified:false,exactSpainSeedEUR:null,exactSpainAverageHitEUR:null,exactSpainHitProbability:null,breakEvenJackpotEUR:null,currentPositiveEvProven:false,researchPriority:'LOWER_UNTIL_EXACT_FEED_GAME_IDENTITY_LOCAL_RESET_LEVEL_AND_HIT_LAW_RESOLVED',economicPromotionAllowed:false,realMoneyAllowed:false},
  guards:{publicNoAuthOnly:true,noCookies:true,noLogin:true,noGameLaunch:true,noBetting:true,noExternalSeedSubstitution:true,requiredContributionScaleIsNotThreshold:true,noLinearMeterEvAssumption:true,noResetAttributionWithoutTriggeringGame:true,noResetCountAsBouncyPHit:true,unverifiedTopologyNeverPromoted:true,sequentialCrossOperatorSamplesNotAtomic:true,realMoneyAllowed:false}
};
