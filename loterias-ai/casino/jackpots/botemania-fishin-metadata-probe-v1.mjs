...
const headers={accept:'application/json','content-type':'application/json',venture:VENTURE,referer:REFERER,origin:'https://www.botemania.es','user-agent':'loterias-ai-botemania-fishin-metadata-probe/1.2'};
...
const imageVariants=[...new Set(games.flatMap(g=>Array.isArray(g.imageVariants)?g.imageVariants:[]).filter(x=>typeof x==='string'&&/^https?:\/\//i.test(x)))];
const out={version:'botemania-fishin-metadata-probe-v1.2',generatedAt:new Date().toISOString(),endpoint:ENDPOINT,ventureHeader:VENTURE,gameSlug:'fishin-frenzy-jackpot-king',probes,games,imageVariants,decision:{metadataRecovered:games.length>0,launchLinkRecovered:games.some(g=>typeof g.link==='string'&&g.link.length>0),officialImageVariantRecovered:imageVariants.length>0,preferredOfficialImageVariant:imageVariants.find(x=>/-05-/i.test(x))||imageVariants[0]||null,exactSpainMbwbRecovered:false,realMoneyAllowed:false},guards:{publicGraphqlOnly:true,knownWebsiteFieldsOnly:true,noIntrospection:true,noAuthentication:true,noCookies:true,noMutation:true,noBetting:true,realMoneyAllowed:false}};
fs.mkdirSync('loterias-ai/casino/jackpots/evidence',{recursive:true});fs.writeFileSync(OUT,JSON.stringify(out,null,2)+'\n');
console.log(JSON.stringify({games:games.map(g=>({source:g.source,id:g.id,title:g.title,link:g.link,providerId:g.providerId,imageSlug:g.imageSlug,imageVariants:g.imageVariants,jackpot:g.jackpot})),imageVariants,errors:probes.map(p=>({name:p.name,httpStatus:p.httpStatus,errors:p.errors})),decision:out.decision},null,2));
