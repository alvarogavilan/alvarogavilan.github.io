...
const UA='loterias-ai-winfall-provider-network-metadata/1.0';
...
export function findStateObjects(html){
  ...
  for(const marker of markers){
    ...
    while(hits.length<12){
      ...
      const look=html.slice(idx,Math.min(html.length,idx+300));
      const brace=look.indexOf('{');if(brace<0)continue;
      const start=idx+brace;let depth=0,inString=false,escape=false,end=-1;
      const HARD_CAP=400000;
      for(let i=start;i<html.length&&i-start<HARD_CAP;i++){
        const ch=html[i];
      ...
      if(end<0)continue;
      const raw=html.slice(start,end);let parsed=null;try{parsed=JSON.parse(raw)}catch{}
      hits.push({marker,index:idx,bytes:raw.length,parsed});
    }
  }
  return hits;
}
...
export function extractSemanticPrimitives(value,p='$',out=[],depth=0){
  ...
  if(value&&typeof value==='object'){
    for(const [k,x] of Object.entries(value)){
      ...
      else if(SEMANTIC_KEY_RE.test(next)&&x!==null&&x!==undefined){
        const raw=String(x);if(raw.length<=500)out.push({path:next,key:k,value:raw,strongKey:STRONG_KEY_RE.test(k)||STRONG_KEY_RE.test(next)});
      }
    }
  }
  return out;
...
function metadataPrimitives(g={}){
  const out=[];
  for(const k of ['id','link','providerId','authorName','categoryId','subCategoryId','imageSlug','customRegistrationUrl']){
    const v=g?.[k];if(v!==null&&v!==undefined&&String(v).trim())out.push({path:`$.metadata.${k}`,key:k,value:String(v),strongKey:STRONG_KEY_RE.test(k)});
  }
...
export function compareSharedSemantic(rows,{targets=TARGETS,controls=CONTROLS}={}){
  ...
  for(const v of common){
    ...
    const strong=evidence.every(e=>e.matches.some(m=>m.strongKey));
    if(strong)candidates.push({value:v,evidence});
  }
  return candidates;
}
...
export async function runProbe(){
  ...
  for(const slug of ALL){
    ...
    const cg=c.body?.data?.contentfulGame||null,pg=p.body?.data?.pageOrGame?.game||null;
    const metadata=cg||pg||{};
    const blobs=findStateObjects(page.text);
    const semantic=[...metadataPrimitives(metadata)];
  ...
  const targetProviders=[...new Set(rows.filter(r=>r.role==='TARGET').map(r=>r.metadata.providerId).filter(Boolean))];
  const controlsProviders=[...new Set(rows.filter(r=>r.role==='CONTROL').map(r=>r.metadata.providerId).filter(Boolean))];
  const sharedStrongCandidates=compareSharedSemantic(rows);
  const coverage={expectedPages:ALL.length,http200Pages:rows.filter(r=>r.page.httpStatus===200).length,metadataRecovered:rows.filter(r=>r.metadata.id||r.metadata.providerId).length,allComplete:rows.every(r=>r.page.httpStatus===200&&r.graphql.contentfulHttpStatus===200&&r.graphql.pageOrGameHttpStatus===200)};
  const out={version:'winfall-provider-network-metadata-v1',generatedAt:new Date().toISOString(),operator:'botemania-es',scope:{targets:TARGETS,controls:CONTROLS},coverage,providerComparison:{targetProviders,controlsProviders,providerExclusiveToTargets:targetProviders.filter(x=>!controlsProviders.includes(x))},rows,comparison:{sharedStrongTargetOnlySemanticCandidates:sharedStrongCandidates},decision:{providerFamilyVerified:targetProviders.length===1&&targetProviders[0]==='roxor-gaming',sharedNetworkConfigCandidate:sharedStrongCandidates.length===1?sharedStrongCandidates[0].value:null,sharedNetworkConfigVerified:false,exactLiveIdVerified:false,nextStep:sharedStrongCandidates.length===1?'REPLICATE_SHARED_METADATA_SIGNATURE_AND_CONNECT_TO_LIVE_FEED_WITH_INDEPENDENT_EVIDENCE':coverage.allComplete?'NO_EXCLUSIVE_STRONG_NETWORK_METADATA_VALUE; PROVIDER_FAMILY_ONLY; MOVE_TO_ROXOR_PUBLIC_GAME_RESOURCE_CONFIG_OR_PROSPECTIVE_SHARED_RESET_CORRELATION':'INCOMPLETE_METADATA_COVERAGE_RETRY_WITHOUT_NEGATIVE',economicPromotionAllowed:false,realMoneyAllowed:false},guards:{knownGraphqlFieldsOnly:true,noIntrospection:true,noAuthentication:true,noCookies:true,noMutation:true,noGameLaunch:true,noBetting:true,providerIdAloneNeverIdentity:true,controlProviderOverlapExcluded:true,onlyStrongSemanticKeysEligibleForNetworkCandidate:true,singleRunNeverVerifiesNetworkConfig:true,noLiveIdFromMetadataWithoutIndependentBinding:true,realMoneyAllowed:false}};
  fs.mkdirSync(path.dirname(OUT),{recursive:true});fs.writeFileSync(OUT,JSON.stringify(out,null,2)+'\n');
  console.log(JSON.stringify({coverage,providerComparison:out.providerComparison,candidates:sharedStrongCandidates,decision:out.decision,rows:rows.map(r=>({slug:r.slug,role:r.role,metadata:r.metadata,hydration:r.hydration,semantic:r.semantic.filter(x=>x.strongKey).slice(0,40)}))},null,2));
  return out;
}
