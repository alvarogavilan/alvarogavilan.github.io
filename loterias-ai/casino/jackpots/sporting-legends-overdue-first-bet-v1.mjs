...
export function evaluateSportingLegendsOverdueFirstBet({
  ...
  betfairFirstBetAfterDeadlineRuleVerified=false,
  ...
  const guards={
    ...
    feedMustCrossDeadlineWithSameBinding:true,
    winCountMustRemainUnchanged:true,
    jackpotMustNotReset:true,
    raceProbabilityMustBeProspectivelyValidated:true,
    realMoneyAllowed:false,
  };
  const fail=(reason,extra={})=>({version:'sporting-legends-overdue-first-bet-v1',decision:'NO_PLAY',valid:false,reason,guards,...extra});
  if(!before||!after)return fail('MISSING_CROSS_DEADLINE_SNAPSHOTS');
  ...
  if(!betfairFirstBetAfterDeadlineRuleVerified)return fail('BETFAIR_FIRST_BET_RULE_NOT_VERIFIED');
  ...
  if(!(beforeTs<=deadline&&afterTs>deadline))return fail('SNAPSHOTS_DO_NOT_BRACKET_DEADLINE');
  ...
  const overdueUnawardedVerified=true;
  const publishedConditionalNextBetTrigger=true;
  ...
  return {
    version:'sporting-legends-overdue-first-bet-v1',
    decision:'NO_PLAY',
    valid:true,
    reason:probabilityGate?'CONDITIONAL_EV_SCREEN_PASSED_EXECUTION_STILL_GUARDED':'OVERDUE_UNAWARDED_STATE_VERIFIED_RACE_GATE_OPEN',
    overdueUnawardedVerified,
    publishedConditionalNextBetTrigger,
