...
const rows=(Array.isArray(census.rows)?census.rows:[]).map(r=>{
  ...
  const researchEligible=usable&&r.stateDependentProbability===true&&r.probabilityProportionalToTotalBet===true;
  ...
  return {
    ...
    researchEligible,
    rankingTier:researchEligible&&sharedCorroboration?'A_EXPLICIT_SHARED_STATE_DEPENDENT':researchEligible?'B_STATE_DEPENDENT_SHARED_NOT_PAGE_EXPLICIT':'C_NOT_PRIMARY'
  };
});
const tierRank={A_EXPLICIT_SHARED_STATE_DEPENDENT:0,B_STATE_DEPENDENT_SHARED_NOT_PAGE_EXPLICIT:1,C_NOT_PRIMARY:2};
...
const out={
  version:'botemania-jpk-all-games-priority-v1',generatedAt:new Date().toISOString(),operator:'botemania-es',
  purpose:'RESEARCH_PRIORITY_ONLY_NOT_WAGER_RECOMMENDATION',
  method:'Rank every parsed Botemania Jackpot King title by how many multiples of its published active jackpot contribution would be needed to bridge base RTP to 100%, then prefer titles whose public page confirms state-dependent, stake-proportional jackpot probability and explicit shared-network context. This is not a state-specific EV calculation.',
...
console.log(JSON.stringify({gamesScanned:out.gamesScanned,primary:out.decision.currentPrimaryResearchGame,beatsFishin:beatsFishin.map(x=>x.slug)},null,2));
