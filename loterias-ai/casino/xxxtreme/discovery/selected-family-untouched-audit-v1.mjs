...
const ECONOMICS='loterias-ai/casino/xxxtreme/economics/rules-v1.json';
...
const economics=JSON.parse(fs.readFileSync(ECONOMICS,'utf8'));
...
if(economics.version!=='xxxtreme-economics-rules-v1'||economics.baseStraightUp?.profitPayout!=='19:1'||Number(economics.baseStraightUp?.grossReturnX)!==20||Number(economics.baseStraightUp?.baseBreakEvenHitRate)!==0.05||economics.safety?.realMoneyAllowed!==false)throw new Error('XXXtreme economics contract drift');
...
const P0=1/37,ALPHA=0.00125,BASE_GROSS_X=Number(economics.baseStraightUp.grossReturnX),BASE_BREAK_EVEN=Number(economics.baseStraightUp.baseBreakEvenHitRate);
const candidates=[];
...
const payload={
  ...
  sourceContract:{economicsRulesVersion:economics.version,economicsRulesPath:ECONOMICS,baseStraightUpProfitPayout:economics.baseStraightUp.profitPayout,baseGrossReturnX:BASE_GROSS_X,lightningComparisonBaseProfitPayout:economics.comparison.lightningBaseProfitPayout,conservativeBaseBreakEvenHitRate:BASE_BREAK_EVEN,multiplierGrossReturnInterpretation:'Use source lucky multiplier x as gross returned units for paper accounting; operator settlement semantics still require deployment-grade verification before any monetary claim.'},
