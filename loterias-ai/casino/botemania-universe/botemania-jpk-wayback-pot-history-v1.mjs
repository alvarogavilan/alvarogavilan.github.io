...
// NOTE: byTier[tier] interleaves candidates from every game whose slug matches
// /jackpot-king/i, sorted only by timestamp. Blueprint's Royal/Regal/Jackpot King
// pots MAY be shared operator-wide across titles, but that has not been
// independently validated for Botemania (see issue #168 guard: "No mezclar
// juegos/redes si no hay evidencia de red compartida"). Every drop candidate is
// therefore tagged with sameSlug so downstream consumers can tell a same-game
// drop from a cross-game timestamp adjacency that may just reflect two
// different, unrelated pots.
const drops={};for(const [tier,arr] of Object.entries(byTier)){const ev=[];for(let i=1;i<arr.length;i++){const p=arr[i-1],q=arr[i];if(q.amountEUR<p.amountEUR*0.9)ev.push({previous:p,current:q,dropEUR:Number((p.amountEUR-q.amountEUR).toFixed(2)),sameSlug:p.slug===q.slug,classification:'HISTORICAL_DISCOVERY_DROP_ONLY'});}drops[tier]=ev;}
const dropCrossGameCounts=Object.fromEntries(Object.entries(drops).map(([k,v])=>[k,v.filter(x=>!x.sameSlug).length]));
const out={version:'botemania-jpk-wayback-pot-history-v1',generatedAt:new Date().toISOString(),operator:'botemania-es',gameCount:games.length,gameSummaries,observations,candidateSeries:byTier,historicalDropCandidates:drops,summary:{totalSnapshotsIndexed:gameSummaries.reduce((s,x)=>s+(x.uniqueSnapshots||0),0),snapshotsFetched:observations.length,snapshotsWithPotCandidates:candidateRows.length,tierCandidateCounts:Object.fromEntries(Object.entries(byTier).map(([k,v])=>[k,v.length])),dropCandidateCounts:Object.fromEntries(Object.entries(drops).map(([k,v])=>[k,v.length])),dropCrossGameCounts},interpretation:'Wayback-derived jackpot amount candidates are discovery evidence only. They may contain stale/display/copy values and cannot promote real-money gates without independent validation. Drop candidates where sameSlug=false compare two DIFFERENT games and must not be read as a reset/hazard event for either game individually unless a shared-network validation confirms the tier is pooled across those titles.',guards:{publicWaybackOnly:true,operatorPagesOnly:true,noFutureLeakage:true,discoveryOnly:true,noAutomaticResetClassification:true,noCrossGameResetClassificationWithoutNetworkValidation:true,noBetting:true,realMoneyAllowed:false}};
