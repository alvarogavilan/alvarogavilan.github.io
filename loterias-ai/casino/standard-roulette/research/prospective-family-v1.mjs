...
const venuePolicy=JSON.parse(fs.readFileSync('loterias-ai/casino/official-venue-policy-v1.json','utf8'));
...
if(addendum?.operationalRule?.doNotPromoteFromAuxiliaryFeed!==true||venuePolicy?.evidencePolicy?.thirdPartyOutcomeFeedMayPromoteEconomicClaimByItself!==false)throw new Error('official venue promotion policy drift');
...
for(const stream of freeze.streams){
  const venue=approved.get(stream.id);if(!venue?.officialAvailabilityConfirmed||venue?.currentCollectorPromotionEligible!==false)throw new Error(`${stream.id} venue approval drift`);
  ...
  if(rows.some(r=>r.promotionEligibleFromThisSource!==false||r.sourceRole!=='AUXILIARY_RESEARCH_ONLY'))throw new Error(`${stream.id} auxiliary source classification missing`);
  ...
  let final=null;if(used===N){const candidates=ids.map(id=>{const hits=stats[id].hits,rate=hits/N,p=binomTail(N,hits,p0),wl=wilsonLower(hits,N),baseROI=rate*gross-1,statisticalPass=p<alpha,economicPass=rate>be&&wl>be,researchPass=statisticalPass&&economicPass;return{id,hits,hitRate:rate,pValue:p,wilson95Lower:wl,baseROI,statisticalPass,economicPass,researchPass,officialVenueEvidenceSatisfied:false,promotionPass:false};});final={fixedBoundaryRounds:N,candidates,anyAuxiliaryResearchEconomicSignal:candidates.some(x=>x.researchPass),anyDirectEconomicSignal:false,interpretation:candidates.some(x=>x.researchPass)?'AUXILIARY_RESEARCH_SIGNAL_ONLY_OFFICIAL_POKERSTARS_OR_PLAYUZU_REPLICATION_REQUIRED':'NO_FROZEN_SELECTOR_PASSED_FIXED_RESEARCH_GATE'};}
  const status={version:'standard-roulette-prospective-family-v1-status',generatedAt:new Date().toISOString(),streamId:stream.id,freezeVersion:freeze.version,venue:{officialGameName:venue.officialGameName,officialOperator:venue.officialOperator,availabilityConfirmed:true},sourceEvidence:{role:'AUXILIARY_RESEARCH_ONLY',operatorOfficial:false,promotionEligibleByItself:false,officialVenueReplicationRequired:true},progress:{postBarrierRows:rows.length,warmupRows:Math.min(2,rows.length),roundsScored:used,fixedBoundaryRounds:N,roundsRemaining:Math.max(0,N-used)},disclosure:{policy:'ADMINISTRATIVE_COUNTS_ONLY',candidatePerformanceHidden:used<N,candidateRankingHidden:used<N,pValuesHidden:used<N,roiHidden:used<N},final,gates:{fixedBoundaryComplete:used===N,auxiliaryResearchSignal:final?.anyAuxiliaryResearchEconomicSignal===true,directEconomicSignal:false,officialVenueEvidenceSatisfied:false,officialVenueReplicationRequired:true,automaticBettingAllowed:false,realMoneyAllowed:false},economics:{straightUpProfitPayout:'35:1',baseGrossReturnX:gross,baseEconomicBreakEvenHitRate:be},guards:{first5000Only:true,noRetuning:true,noOptionalStopping:true,postBoundaryCannotRescue:true,perCandidateAlpha:alpha,preBarrierPredictiveStateUsed:false,auxiliaryFeedCannotPromote:true,realMoneyAllowed:false,realStakeEUR:0}};
