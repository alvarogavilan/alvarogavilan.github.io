...
const VERSION='analyze-betfair-scarab-har-v1';
...
export function analyzeBetfairScarabHarText(raw,{sourceName='betfair-scarab.har'}={}){
  let har;try{har=JSON.parse(raw);}catch(error){return fail('HAR_PARSE_FAILED',{message:String(error?.message||error)});}
  let analysis;try{analysis=extractBetfairScarabHarCandidate(har,{sourceName});}catch(error){return fail('HAR_ANALYSIS_FAILED',{message:String(error?.message||error)});}
...
export function main(argv=process.argv.slice(2)){
  const file=argv[0];
  if(!file||file==='--help'||file==='-h'){process.stdout.write('Usage: node loterias-ai/scripts/analyze-betfair-scarab-har.mjs <capture.har>\n');return file?0:2;}
  try{const out=analyzeBetfairScarabHarText(fs.readFileSync(file,'utf8'),{sourceName:path.basename(file)});process.stdout.write(`${JSON.stringify(out,null,2)}\n`);return out.ok?0:1;}catch(error){process.stdout.write(`${JSON.stringify(fail('HAR_READ_FAILED',{file:path.basename(file),message:String(error?.message||error)}),null,2)}\n`);return 1;}
}
