...
const manifest={generatedAt:new Date().toISOString(),schemaVersion:4,storage:'canonical JSON partitions; legacy Quiniela seasonStart files are season partitions, official year files are calendar-year partitions',validationSemantics:'fail-closed exact token matching; INVALID/PENDING/UNVERIFIED/CONFLICT never count as validated',games:gameSummary,totals:{games:Object.keys(gameSummary).length,partitions:partitions.length,records:partitions.reduce((s,p)=>s+p.records,0),economicsRecords:partitions.reduce((s,p)=>s+p.economicsRecords,0),validatedRecords:partitions.reduce((s,p)=>s+p.validatedRecords,0)},partitions};
