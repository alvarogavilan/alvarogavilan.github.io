...
for(const [index,target] of targets.slice(0,LIMIT).entries()){
  ...
  try{
    ...
    const storedComplementary=Number(r.result?.complementary);
    const storedReintegro=Number(r.result?.reintegro);
    ...
    if((mainParsed&&!fields.main)||(complementaryParsed&&Number.isInteger(storedComplementary)&&!fields.complementary)||(reintegroParsed&&Number.isInteger(storedReintegro)&&!fields.reintegro)){
      conflicts.push({
        date:r.drawDate,
    ...
    for(const check of ['official-fechav3-exact-date','official-main-cross-check','official-complementary-cross-check','official-reintegro-cross-check'])if(!checks.includes(check))checks.push(check);
    r.verification={
      ...
      officialCrossCheck:{
        ...
        sourceUrl:url,
        checkedAt:now,
        officialDrawId:row.id_sorteo||null,
        exactDate:true,
        fields,
        complete,
    ...
    if(complete)validated++;else partial++;
  }catch(error){
    failed++;
...
for(const conflict of conflicts)conflictMap.set(conflict.date,conflict);
...
const out={
  generatedAt:now,
  gameId:'primitiva',
  source:'SELAE /servicios/fechav3 game_id=LAPR',
  attempted,
  validated,
  partial,
  empty,
  ...
  openConflicts:conflictDoc.conflicts.length,
  totalRecords:total,
  componentCrossChecked,
  officiallyValidated,
  remaining:Math.max(0,targets.length-validated-partial),
  qualityPass:conflictDoc.conflicts.length===0,
  guards:{
