...
for(const [i,date] of dates.entries()){
  ...
  try{
    const r=await fetch(url,{redirect:'follow',signal:AbortSignal.timeout(20000),headers:{accept:'text/html,application/rss+xml,*/*','user-agent':'LoteriasAI-SELAE-payout-cert/1.0'}});
    ...
    if(!complete||!resultVerified){failures.push({date,reason:!complete?'PAYOUT_PARSE_INCOMPLETE':'CANONICAL_RESULT_NOT_FULLY_SELAE_VALIDATED',url});continue;}
...
const payload={version:'bonoloto-official-payouts-recent-v1',generatedAt:new Date().toISOString(),sourcePolicy:'SELAE_ONLY',targetDates:dates.length,summary:{targetDates:dates.length,certified:rows.length,failed:failures.length,complete:rows.length===dates.length&&failures.length===0},rows,failures,guards:{exactCanonicalDate:true,canonicalResultMustAlreadyBeSELAEValidated:true,allFiveMainPrizeCategoriesPlusReintegroRequired:true,noThirdPartyPayouts:true,noBacktestMutation:true,noBetting:true,realMoneyAllowed:false}};
fs.writeFileSync(OUT,JSON.stringify(payload,null,2)+'\n');console.log(JSON.stringify(payload.summary,null,2));if(!payload.summary.complete)process.exitCode=2;
