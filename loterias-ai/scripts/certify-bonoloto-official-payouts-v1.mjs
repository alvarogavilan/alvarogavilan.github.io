...
const officialUrl=date=>{const [y,m,d]=date.split('-');return `https://www.loteriasyapuestas.es/es/bonoloto/escrutinios/bonoloto-premios-y-ganadores-del-${d}-de-${MONTHS[Number(m)-1]}-de-${y}.formatoRSS`;};
const clean=s=>s.replace(/<script[\s\S]*?<\/script>/gi,' ').replace(/<style[\s\S]*?<\/style>/gi,' ').replace(/<[^>]+>/g,' ').replace(/&nbsp;|&#160;/gi,' ').replace(/&euro;|&#8364;/gi,'€').replace(/&ordf;|&#170;/gi,'ª').replace(/&ordm;|&#186;/gi,'º').replace(/&amp;/gi,'&').replace(/\s+/g,' ').trim();
...
for(const [i,date] of dates.entries()){
  ...
  const stored=rec?.source?.sourceUrl;
  const url=stored&&/^https:\/\/www\.loteriasyapuestas\.es\/es\/bonoloto\/escrutinios\//.test(stored)?stored:officialUrl(date);
  try{
    const r=await fetch(url,{redirect:'follow',signal:AbortSignal.timeout(20000),headers:{accept:'text/html,application/rss+xml,*/*','user-agent':'LoteriasAI-SELAE-payout-cert/1.2'}});
    ...
    const payoutComplete=Object.values(payouts).every(Boolean);
    ...
    if(!payoutComplete||!resultVerified){failures.push({date,reason:!payoutComplete?'PAYOUT_PARSE_INCOMPLETE':'CANONICAL_RESULT_NOT_FULLY_SELAE_VALIDATED',url,urlMode:stored===url?'STORED_OFFICIAL':'DETERMINISTIC_OFFICIAL_FROM_DATE',parsedCategories:Object.fromEntries(Object.entries(payouts).map(([k,v])=>[k,Boolean(v)]))});continue;}
    rows.push({date,url,urlMode:stored===url?'STORED_OFFICIAL':'DETERMINISTIC_OFFICIAL_FROM_DATE',httpStatus:r.status,bodySha256:crypto.createHash('sha256').update(body).digest('hex'),officialResult:{main:rec.result.main.map(Number),complementary:Number(rec.result.complementary),reintegro:Number(rec.result.reintegro)},payouts:Object.fromEntries(Object.entries(payouts).map(([k,v])=>[k,{winners:v.winners,prizeEUR:v.prizeEUR}])),resultVerification:'OFFICIAL_SELAE_VALIDATED',payoutVerification:'OFFICIAL_SELAE_PAGE_PARSED'});
  }catch(e){failures.push({date,reason:String(e?.message||e),url,urlMode:stored===url?'STORED_OFFICIAL':'DETERMINISTIC_OFFICIAL_FROM_DATE'});}
  if(i%10===9)await new Promise(r=>setTimeout(r,300));
}
const complete=rows.length===dates.length&&failures.length===0;
const payload={version:'bonoloto-official-payouts-recent-v1',generatedAt:new Date().toISOString(),sourcePolicy:'SELAE_ONLY',targetDates:dates.length,summary:{targetDates:dates.length,certified:rows.length,failed:failures.length,complete,constructedOfficialUrls:rows.filter(x=>x.urlMode==='DETERMINISTIC_OFFICIAL_FROM_DATE').length},rows,failures,tournamentGate:{officialV3MayRun:complete,reason:complete?'ALL_TARGET_DATES_SELAE_CERTIFIED':'BLOCKED_UNTIL_ALL_TARGET_DATES_CERTIFIED'},guards:{exactCanonicalDate:true,deterministicSELAEUrlFromDrawDateAllowed:true,canonicalResultMustAlreadyBeSELAEValidated:true,allFiveMainPrizeCategoriesPlusReintegroRequired:true,noThirdPartyPayouts:true,noBacktestMutation:true,incompleteDiagnosticsPersisted:true,noBetting:true,realMoneyAllowed:false}};
