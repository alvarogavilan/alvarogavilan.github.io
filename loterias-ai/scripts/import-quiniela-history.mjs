...
const parseDate=(s,seasonStart)=>{const m=String(s).match(/(\d{1,2})[\/\-.](\d{1,2})[\/\-.](\d{2,4})/);if(!m)return null;let y=Number(m[3]);if(m[3].length===2){const yy=y, a=seasonStart, b=seasonStart+1;if(yy===a%100)y=a;else if(yy===b%100)y=b;else {const candidates=[1900+yy,2000+yy];y=candidates.sort((x,z)=>Math.min(Math.abs(x-a),Math.abs(x-b))-Math.min(Math.abs(z-a),Math.abs(z-b)))[0];}}const month=Number(m[2]),day=Number(m[1]);if(y<seasonStart||y>seasonStart+1||month<1||month>12||day<1||day>31)return null;return `${y}-${String(month).padStart(2,'0')}-${String(day).padStart(2,'0')}`};
...
for(let y=1972;y<=2025;y++){
  ...
  try{
    ...
    for(const tr of html.match(/<tr[\s\S]*?<\/tr>/gi)||[]){
      ...
      let date=null;for(const c of cells){const d=parseDate(c,y);if(d){date=d;break}}
