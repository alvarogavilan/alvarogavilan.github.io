...
for(const e of ledger.entries){if(e.model?.window!==window||e.model?.ticketsPerDraw!==ticketsPerDraw||Number(e.realStakeEUR||0)!==0)throw new Error(`Quinigol ledger model drift ${e.entryId}`)}
...
ledger.guards={first30Only:true,noRetuning:true,noOptionalStopping:true,post30CannotRescue:true,automaticBettingAllowed:false,realMoneyAllowed:false,realStakeEUR:0};
