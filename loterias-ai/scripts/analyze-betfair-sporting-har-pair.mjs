...
function safeStakeReview(v){return {valid:v?.valid===true,reason:v?.reason||null,stakeAtDecisionExactVerified:v?.stakeAtDecisionExactVerified===true,servedStakeMenuSemanticsVerified:v?.servedStakeMenuSemanticsVerified===true,servedTotalStakeValuesEUR:Array.isArray(v?.servedTotalStakeValuesEUR)?v.servedTotalStakeValuesEUR:null,selectedStakeEUR:v?.selectedStakeEUR??null,minimumServedTotalStakeEUR:v?.minimumServedTotalStakeEUR??null,reviewCandidate:v?.reviewCandidate?{endpoint:safeEndpoint(v.reviewCandidate.endpoint),normalizedKey:v.reviewCandidate.normalizedKey||null,objectPath:v.reviewCandidate.objectPath||null}:null};}
function safeEvaluation(r,requestedStakeEUR){
  ...
  return {
    ...
    overdue:{followingDayUnawardedVerified:f.followingDayUnawardedVerified===true,nextEligibleNetworkBetGuaranteedJackpot:f.nextEligibleNetworkBetGuaranteedJackpot===true,exactBetfairSpainTickerImsBindingVerified:f.exactBetfairSpainTickerImsBindingVerified===true,deadlineEpochSeconds:f.deadlineEpochSeconds??null,beforeLeadSeconds:f.beforeLeadSeconds??null,afterLagSeconds:f.afterLagSeconds??null,zeroEligibleArrivalWindowSeconds:f.zeroEligibleArrivalWindowSeconds??null,feedAgeSeconds:f.feedAgeSeconds??null,maxFeedAgeSeconds:f.maxFeedAgeSeconds??null,winCount:f.winCount??null,currentDailyJackpotEUR:f.currentDailyJackpotEUR??null,currentDailyAmountExactVerifiedFromValidatedServerSnapshot:r?.currentDailyAmountExactVerifiedFromValidatedServerSnapshot===true},
    raceGate:{requestedStakeEUR:requestedStakeEUR??null,reviewedStakeEUR:f.stakeEUR??null,stakeAtDecisionExactVerified:r?.stakeAtDecisionExactVerifiedFromCodeOwnedReview===true,conservativeBaseRtpPct:f.conservativeBaseRtpPct??null,breakEvenFirstBetProbability:f.breakEvenFirstBetProbability??null,structuredProspectiveRaceEvidenceVerified:f.executionGates?.structuredProspectiveRaceEvidenceVerified===true,raceExecutionAssumptionsVerified:f.executionGates?.raceExecutionAssumptionsVerified===true,raceConfidenceVerified:f.executionGates?.raceConfidenceVerified===true,raceLedgerIdentityVerified:f.executionGates?.raceLedgerIdentityVerified===true,raceWindowBudgetVerified:f.executionGates?.raceWindowBudgetVerified===true,measuredActionLatencyVerified:f.executionGates?.measuredActionLatencyVerified===true,prospectiveDryRunCycleVerified:f.executionGates?.prospectiveDryRunCycleVerified===true,conditionalPositiveEvScreenPassed:f.conditionalPositiveEvScreenPassed===true,executionGateClosed:f.executionGateClosed===true},
    underlyingScientificReason:f.reason||r?.reason||null,
  };
}

export function analyzeSafeHarPairText(beforeRaw,afterRaw,{beforeSourceName='before.har',afterSourceName='after.har',decisionNowEpochSeconds,stakeEUR=null,stakeReviewCommit=null}={}){
...
export function main(argv=process.argv.slice(2)){
  ...
  if(!beforeFile||!afterFile||beforeFile==='--help'||beforeFile==='-h'){process.stdout.write(`${usage()}\n`);return beforeFile?0:2;}
  const decisionIndex=argv.indexOf('--decision-now-epoch');const decisionNowEpochSeconds=decisionIndex>=0?finite(argv[decisionIndex+1]):null;
  if(decisionNowEpochSeconds===null){process.stdout.write(`${JSON.stringify(fail('EXPLICIT_DECISION_TIME_REQUIRED',{usage:usage()}),null,2)}\n`);return 2;}
  const stakeIndex=argv.indexOf('--stake-eur');const stakeEUR=stakeIndex>=0?finite(argv[stakeIndex+1]):null;
  if(stakeIndex>=0&&stakeEUR===null){process.stdout.write(`${JSON.stringify(fail('INVALID_STAKE_EUR'),null,2)}\n`);return 2;}
  const reviewIndex=argv.indexOf('--stake-review-commit');const stakeReviewCommit=reviewIndex>=0?String(argv[reviewIndex+1]||''):null;
  if(reviewIndex>=0&&!/^[0-9a-f]{40}$/i.test(stakeReviewCommit||'')){process.stdout.write(`${JSON.stringify(fail('INVALID_STAKE_REVIEW_COMMIT'),null,2)}\n`);return 2;}
  try{
    const result=analyzeSafeHarPairText(fs.readFileSync(beforeFile,'utf8'),fs.readFileSync(afterFile,'utf8'),{beforeSourceName:path.basename(beforeFile),afterSourceName:path.basename(afterFile),decisionNowEpochSeconds,stakeEUR,stakeReviewCommit});
