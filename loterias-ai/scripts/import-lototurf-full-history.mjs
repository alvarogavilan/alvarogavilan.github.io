...
const isoDate=s=>{const m=String(s).match(/(\d{1,2})[\/\-.](\d{1,2})[\/\-.](\d{4})/);if(!m)return null;return `${m[3]}-${m[2].padStart(2,'0')}-${m[1].padStart(2,'0')}`};
const rows=[],rejected=[];
for(const [idx,line] of text.split(/\r?\n/).entries()){
 const date=isoDate(line);if(!date)continue;
 const after=line.slice(Math.max(0,line.search(/\d{1,2}[\/\-.]\d{1,2}[\/\-.]\d{4}/)+10));
 const nums=(after.match(/\b\d{1,2}\b/g)||[]).map(Number);
 if(nums.length<8){rejected.push({idx:idx+1,date,reason:'less-than-8-numbers'});continue}
 const main=nums.slice(0,6),horse=nums[6],reintegro=nums[7];
 if(new Set(main).size!==6||main.some(n=>n<1||n>31)||horse<1||horse>20||reintegro<0||reintegro>9){rejected.push({idx:idx+1,date,reason:'range-or-duplicate',main,horse,reintegro});continue}
...
const summary={generatedAt:new Date().toISOString(),gameId:'lototurf',source:URL,records:dedup.length,earliest:dedup[0]?.drawDate,latest:dedup.at(-1)?.drawDate,years:years.length,rejected:rejected.length};fs.mkdirSync('loterias-ai/data/archive/_meta',{recursive:true});fs.writeFileSync('loterias-ai/data/archive/_meta/lototurf-import.json',JSON.stringify(summary,null,2)+'\n');console.log(JSON.stringify(summary,null,2));
