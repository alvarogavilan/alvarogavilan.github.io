...
const base={version:'euromillones-v199-complete-5plus2-evaluation-v1',generatedAt:new Date().toISOString(),targetDrawDate:targetDate,freezeFingerprintSha256:freeze.fingerprintSha256,retuningPerformed:false,selectionUsedTargetResult:false,realMoneyPass:false,realStakeEUR:0,bettingRecommendation:false};
let out;
if(!target) out={...base,status:'WAITING_OFFICIAL_ARCHIVE',ticket:freeze.tickets[0],next:'wait for official archived target draw; do not alter freeze'};
else {
  ...
  if(nums.length!==5||stars.length!==2) out={...base,status:'WAITING_COMPLETE_OFFICIAL_RESULT',ticket:freeze.tickets[0],archivedTargetFound:true};
  else {
    ...
    out={...base,status:'EVALUATED',ticket:t,winningNumbers:nums,winningStars:stars,numberHits:nHits,starHits:sHits,category:`${nHits}+${sHits}`,full5plus2:nHits===5&&sHits===2,nearFull:nHits>=4||nHits===5,claimAllowed:nHits===5&&sHits===2,interpretation:nHits===5&&sHits===2?'prospective 5+2 matched; requires independent replication before any predictive claim':'record prospective result only; no retuning'};
  }
}
