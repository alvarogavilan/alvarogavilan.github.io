...
for(const f of cfg.feeds){
  ...
  try{const r=await fetch(url,{redirect:'follow',headers:{'user-agent':'LoteriasAI-SourceProbe/1.4'}});const body=await r.text();out.feeds.push({gameId:f.gameId,url,status:r.status,ok:r.ok,bytes:body.length});}
...
const reachable=out.feeds.filter(x=>x.ok).length, jsonOK=out.serviceChecks.filter(x=>x.payloadShape).length;
console.log(`SELAE pages ${reachable}/${out.feeds.length}; fechav3 JSON ${jsonOK}/${out.serviceChecks.length}`);
