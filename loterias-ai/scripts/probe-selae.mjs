...
function scriptSources(s){const a=[];const re=/<script[^>]+src=["']([^"']+)["'][^>]*>/gi;let m;while((m=re.exec(String(s||''))))a.push(m[1]);return [...new Set(a)];}
function inlineScripts(s){const a=[];const re=/<script(?![^>]*\bsrc=)[^>]*>([\s\S]*?)<\/script>/gi;let m;while((m=re.exec(String(s||'')))){const v=m[1].replace(/\s+/g,' ').trim();if(v)a.push(v.slice(0,1200));}return a;}
function endpoint(f,date){return cfg.datedEndpointBase+f.template.replace('{YYYYMMDD}',date);}
for(const f of cfg.feeds){
  const url=endpoint(f,today);
  try{
    const r=await fetch(url,{redirect:'follow',headers:{'user-agent':'LoteriasAI-SourceProbe/1.3'}});
    const body=await r.text();
    out.feeds.push({gameId:f.gameId,url,status:r.status,ok:r.ok,bytes:body.length,looksHtml:/<html|<!doctype/i.test(body),hasContent:body.trim().length>20,scriptSources:scriptSources(body)});
  ...
  if(known){
    ...
    try{
      const kr=await fetch(ku,{redirect:'follow',headers:{'user-agent':'LoteriasAI-ParserProbe/1.2'}});const kb=await kr.text();const plain=htmlToText(kb);
      const parsed=adapter.extractNumeric(f.gameId,plain);const validation=adapter.validateNumericDraw(parsed);
      out.parserChecks.push({gameId:f.gameId,date:known,url:ku,httpStatus:kr.status,bytes:kb.length,plainExcerpt:plain.slice(0,500),htmlStructure:htmlStructure(kb),scriptSources:scriptSources(kb),inlineScripts:inlineScripts(kb),parsed,validation});
