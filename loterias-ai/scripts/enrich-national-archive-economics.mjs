...
const sources={
  primitiva:{url:'https://www.azarysuerte.es/HistoricoPR.php',prefix:'primitiva',parser:parsePrimitiva},
  euromillones:{url:'https://www.azarysuerte.es/HistoricoEM.php',prefix:'euromillones',parser:parseEuromillones},
  bonoloto:{url:'https://www.azarysuerte.es/HistoricoBO.php/',prefix:'bonoloto',parser:parseBonoloto},
};
...
for(const [gameId,cfg] of Object.entries(sources)){
  ...
  for(const [year,ys] of byYear){const path=`loterias-ai/data/archive/${cfg.prefix}/${year}.json`;if(!fs.existsSync(path)){missing+=ys.length;continue;}const doc=JSON.parse(fs.readFileSync(path,'utf8')),map=new Map(ys.map(x=>[x.date,x]));for(const rec of doc.records||[]){const e=map.get(rec.drawDate);if(!e)continue;matched++;rec.economics={ticketCost:gameId==='euromillones'?2.5:gameId==='primitiva'?1:0.5,currency:'EUR',payouts:e.payouts,source:{provider:'Azar y Suerte',url:cfg.url,capturedAt:new Date().toISOString()},validation:{status:'SOURCE_PARSED_PENDING_CROSS_CHECK'}};if(e.complementary!=null)rec.result.complementary=e.complementary;if(e.reintegro!=null)rec.result.reintegro=e.reintegro;if(e.stars)rec.result.stars=e.stars;updated++;}
