...
function canonical(rec,mode='main'){
  ...
  if(legacyOutcomes?.length>=14){
    if(mode==='main')return stable({signs:legacyOutcomes.slice(0,14)});
    return stable({signs:legacyOutcomes.slice(0,14),pleno15:legacyOutcomes[14]??null});
  }
...
function analyzeMode(rows,mode){
  const seq=rows.map(r=>({date:r.drawDate,key:canonical(r,mode),cost:ticketCost(r)})).filter(x=>x.key).map((x,pos)=>({...x,pos}));
  const occurrences=new Map();for(const x of seq){if(!occurrences.has(x.key))occurrences.set(x.key,[]);occurrences.get(x.key).push(x);}
  ...
  const replayWindows=windows.map(w=>{let hits=0,played=0,knownStake=0,stakeComplete=true;const samples=[];for(let i=1;i<seq.length;i++){const prior=new Set(seq.slice(Math.max(0,i-w),i).map(x=>x.key));played+=prior.size;if(prior.has(seq[i].key)){hits++;if(samples.length<25)samples.push({date:seq[i].date,signature:seq[i].key,activeTickets:prior.size});}const c=seq[i].cost;if(c>0)knownStake+=c*prior.size;else stakeComplete=false;}return{windowPreviousDraws:w,fullHits:hits,ticketsPlayed:played,knownStakeEUR:stakeComplete?Number(knownStake.toFixed(2)):null,samples};});
...
const report={generatedAt:new Date().toISOString(),methodology:'historical-full-hit-hunter-v2',warning:'A historical full hit or repeated winning signature is not evidence of predictive edge. It is catalogued for research and must survive untouched holdout and matched-cost baselines before any real-money gate can change.',games:{}};
