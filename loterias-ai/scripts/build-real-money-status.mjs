...
const games=defs.map(d=>{if(!d.file||!fs.existsSync(`${root}/${d.file}`))return{id:d.id,name:d.name,status:'PENDING',realMoneyPass:false,reason:'Official financial gate not completed'};const j=JSON.parse(fs.readFileSync(`${root}/${d.file}`,'utf8')),h=j.holdout||{},official=j.officialEconomicDraws??null,pass=j.realMoneyPass===true,blocked=j.status==='BLOCKED_DATA'||j.status==='INSUFFICIENT_SAMPLE';return{id:d.id,name:d.name,status:blocked?'BLOCKED':pass?'ELIGIBLE':'REJECTED',realMoneyPass:pass,officialEconomicDraws:official,holdoutDraws:h.draws??j.holdoutDraws??null,holdoutStake:h.stake??null,holdoutReturn:h.return??null,holdoutPL:h.pl??null,holdoutROI:h.roi??null,selected:j.selected??j.winner??{strategy:j.strategy,tickets:j.tickets},baseline:j.randomBaseline??null,source:j.source??null,reason:blocked?(j.reason||'Insufficient official reconstructable data'):pass?'All completed official gates passed':'Completed official financial gate did not pass'}});
const eligible=games.filter(g=>g.status==='ELIGIBLE'),rejected=games.filter(g=>g.status==='REJECTED'),blocked=games.filter(g=>g.status==='BLOCKED'),pending=games.filter(g=>g.status==='PENDING');const out={generatedAt:new Date().toISOString(),policy:{realMoneyRequires:'official economics + locked holdout + positive ROI + matched random baselines + sufficient sample + prospective shadow before allocation',defaultAllocationEUR:0},summary:{eligible:eligible.length,rejected:rejected.length,blocked:blocked.length,pending:pending.length,approvedStakeTodayEUR:0},games};fs.mkdirSync('loterias-ai/data/gates',{recursive:true});fs.writeFileSync('loterias-ai/data/gates/real-money-status.json',JSON.stringify(out,null,2)+'\n');console.log(JSON.stringify(out.summary,null,2));