...
rows=rows.filter(r=>r.drawDate&&Array.isArray(r.result?.main)&&r.result.main.length===cfg.k).sort((a,b)=>String(a.drawDate).localeCompare(String(b.drawDate)));
...
const cache=new Map(),extraCache=new Map();
function featurePack(i,sw,lw){const key=`m|${i}|${sw}|${lw}`;if(cache.has(key))return cache.get(key);const f=Array.from({length:cfg.max+1},()=>[0,0,0,0,0,0]);const sc=new Uint16Array(cfg.max+1),lc=new Uint16Array(cfg.max+1),last=new Int32Array(cfg.max+1);last.fill(-9999);for(let j=Math.max(0,i-lw);j<i;j++)for(const n of rows[j].result.main){lc[n]++;last[n]=j;if(j>=i-sw)sc[n]++}const prev=mainSets[i-1]||new Set();for(let n=1;n<=cfg.max;n++){const sf=sc[n]/Math.max(1,Math.min(sw,i)),lf=lc[n]/Math.max(1,Math.min(lw,i)),gap=Math.min(200,i-last[n])/200;f[n]=[sf,lf,sf-lf,gap,prev.has(n)?1:0,n/cfg.max]}cache.set(key,f);return f}
function extraPack(name,max,base,i,sw,lw){const key=`e|${name}|${i}|${sw}|${lw}`;if(extraCache.has(key))return extraCache.get(key);const vals=Array.from({length:max},(_,q)=>base+q),f=[];for(const v of vals){let s=0,l=0,last=-9999;for(let j=Math.max(0,i-lw);j<i;j++){const a=extras[j][name]||[];if(a.includes(v)){l++;last=j;if(j>=i-sw)s++}}const sf=s/Math.max(1,Math.min(sw,i)),lf=l/Math.max(1,Math.min(lw,i));f.push([v,[sf,lf,sf-lf,Math.min(200,i-last)/200,(extras[i-1]?.[name]||[]).includes(v)?1:0,(v-base+1)/max]])}extraCache.set(key,f);return f}
function extraPredict(name,max,count,base,i,sw,lw,w){const score=[];for(const [v,x] of extraPack(name,max,base,i,sw,lw)){let z=0;for(let q=0;q<6;q++)z+=w[q]*x[q];score.push([z,v])}score.sort((a,b)=>b[0]-a[0]||a[1]-b[1]);return score.slice(0,count).map(x=>x[1]).sort((a,b)=>a-b)}
...
function predict(c,i){const F=featurePack(i,c.sw,c.lw),sc=[];for(let n=1;n<=cfg.max;n++){let z=0;for(let q=0;q<6;q++)z+=c.w[q]*F[n][q];z+=((mix32(c.id^n)%1000)/1000-0.5)*0.0001;sc.push([z,n])}sc.sort((a,b)=>b[0]-a[0]||a[1]-b[1]);const main=sc.slice(0,cfg.k).map(x=>x[1]).sort((a,b)=>a-b),ex={};for(const [name,max,count,base] of cfg.extra)ex[name]=extraPredict(name,max,count,base,i,c.sw,c.lw,c.w);return{main,ex}}
