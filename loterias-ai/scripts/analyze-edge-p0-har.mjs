...
const VERSION='analyze-edge-p0-har-v1.4-nine-lane-passive';
...
function usage(){return `Usage: node loterias-ai/scripts/analyze-edge-p0-har.mjs <apmccoy|frank|ultimate-vp|betfair-regal-riches|magic-nile|scarab|hexbreak3r|golden-egypt|ocean-magic> <capture.har>\n`;}
...
export function analyzeEdgeP0HarText(raw,{lane,sourceName='capture.har'}={}){
  ...
  return {version:VERSION,ok:true,lane,laneLabel:LANES[lane].label,sourceName,analysisVersion:result?.version||result?.analysis?.version||null,closed:safeClosed(lane,result),result,execution:execution(),scientificUse:'Unified local dispatcher for nine passive research lanes. It does not fetch network data, place wagers, approve review candidates or authorize execution. Each lane has a distinct fail-closed summary shape.',hardGuards:{onlineOnly:true,nonPromoOnly:true,offlineOnly:true,passiveHarOnly:true,noNetwork:true,noWagerProbe:true,noAutomaticBetting:true,rawHarNeverEmitted:true,rawResponseBodiesNeverEmitted:true,reviewCandidatesCannotSelfApprove:true,laneSpecificSummaryRequired:true,crossLaneGateTransferForbidden:true,enrachaRegalRichesTitleCollisionCannotEnterBetfairRegalLane:true,goldenEgyptTitleCannotSelfProveIgt:true,realMoneyAllowed:false}};
}
