...
function prefixCounts(rows){
  ...
  for(let i=0;i<rows.length;i++){p[i+1].set(p[i]);for(const n of rows[i].main)p[i+1][n]++;}
  return p;
}
...
function pool(p,i,cfg){return Array.from({length:MAXN},(_,k)=>k+1).sort((a,b)=>haarScore(p,i,b,cfg)-haarScore(p,i,a,cfg)||a-b).slice(0,POOL).sort((a,b)=>a-b);}
function baselinePool(p,i,w=160){return Array.from({length:MAXN},(_,k)=>k+1).sort((a,b)=>meanOccurrence(p,i,b,w)-meanOccurrence(p,i,a,w)||a-b).slice(0,POOL).sort((a,b)=>a-b);}
...
function evaluate(rows,p,cfg,start,end){
  let draws=0,sum=0,bSum=0,h4=0,b4=0,h5=0,b5=0,h6=0,b6=0,wins=0,losses=0,ties=0;const events=[];
...
function twoSidedSignP(w,l){const n=w+l;if(!n)return 1;const m=Math.min(w,l);let term=2**(-n),cdf=term;for(let k=0;k<m;k++){term*=((n-k)/(k+1));cdf+=term;}return Math.min(1,2*cdf);}
function hardAudit(rows,p,cfg,validation,postFreeze){
  const dates=rows.map(r=>r.date),dupes=dates.length-new Set(dates).size;const allEvents=[...validation.events,...postFreeze.events];
  const exactEventRecheck=allEvents.every(e=>{const recomputed=pool(p,e.index,cfg);return JSON.stringify(recomputed)===JSON.stringify(e.pool)&&hit(recomputed,e.truth)===e.hits&&recomputed.length===POOL&&new Set(recomputed).size===POOL;});
  ...
  const signP=twoSidedSignP(w,l),replicated5=validation.hit5Plus>0&&postFreeze.hit5Plus>0,any6=validation.full6>0||postFreeze.full6>0;
  const statisticalAnomaly=signP<0.01&&validation.deltaMeanHits>0&&postFreeze.deltaMeanHits>0;
  const triggered=allEvents.length>0||any6||statisticalAnomaly,integrityPass=dupes===0&&exactEventRecheck&&preDrawCausalityCheck;
...
const rows=load(),p=prefixCounts(rows),cfgs=[];
...
const leader=frozen[0],audit=hardAudit(rows,p,leader.cfg,leader.validation,leader.postFreeze),positiveBoth=leader.validation.deltaMeanHits>0&&leader.postFreeze.deltaMeanHits>0;
