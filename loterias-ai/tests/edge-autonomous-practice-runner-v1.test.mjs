...
assert.ok(r.stakeStateMechanics.filter(x=>x.result.status==='REAL_STAKE_DEPENDENT_MECHANIC_RESEARCH_CANDIDATE').length>=5);
...
assert.equal(r.snakesNormalizedSensitivity[0].observedActiveSegments,10);
assert.equal(r.snakesNormalizedSensitivity[0].additionalProgressPointsNeeded,1);
assert.equal(r.snakesNormalizedSensitivity[0].breakEvenCompletionProbabilityPct,5);
assert.ok(r.findings.some(x=>x.id==='AOTGN_WAYS_OF_THUNDER_PER_BET_STATE'));
assert.ok(r.findings.some(x=>x.id==='SQUEALIN_RICHES_PER_BET_STATE'));
