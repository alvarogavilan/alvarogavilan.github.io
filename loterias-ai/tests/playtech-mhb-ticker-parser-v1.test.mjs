...
assert.equal(PLAYTECH_MHB_TARGETS['aognjp-2'].providerScope,'GLOBAL');
assert.equal(PLAYTECH_MHB_TARGETS['aognjp-3'].providerScope,'GLOBAL');
assert.equal(PLAYTECH_MHB_TARGETS['aognjp-7'].providerScope,'GLOBAL');

const all=parsePlaytechMhbTickerXml(xml,{nowEpochSeconds:1500});
assert.equal(all.rows.length,7);
assert.equal(all.rejected.providerScope,1);
