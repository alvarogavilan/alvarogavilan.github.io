...
import {analyzeSafeDualFeedTexts,parseDualFeedCliArgs} from '../scripts/analyze-betfair-sporting-dual-feed.mjs';

const raw=(timestamp,amount,ght=timestamp+100)=>JSON.stringify({log:{entries:[
  ...
  {startedDateTime:new Date((timestamp+1)*1000).toISOString(),request:{method:'POST',url:'https://webtickers.malmegas.com/webtickers?info=1&casino=bf_es&game=sljp-1&currency=EUR&local=0&token=MODERN_QUERY_SECRET',headers:[{name:'Authorization',value:'Bearer MODERN_AUTH_SECRET'}],postData:{mimeType:'application/json',text:'{"token":"BODY_SECRET"}'}},response:{status:200,content:{mimeType:'application/json',text:JSON.stringify({game:'sljp-1',currency:'EUR',local:0,timestamp,winc:7,amount,guaranteedHitTime:ght,token:'RESPONSE_SECRET'})}}},
]}});

// No option: the first HAR must remain in the file list.
let parsed=parseDualFeedCliArgs(['first.har','second.har']);
assert.equal(parsed.ok,true);
assert.deepEqual(parsed.files,['first.har','second.har']);
assert.equal(parsed.maxCaptureSkewSeconds,5);

// Option may appear after files without dropping either input.
parsed=parseDualFeedCliArgs(['first.har','second.har','--max-skew','2']);
assert.equal(parsed.ok,true);
assert.deepEqual(parsed.files,['first.har','second.har']);
assert.equal(parsed.maxCaptureSkewSeconds,2);

const duplicateOption=parseDualFeedCliArgs(['first.har','--max-skew','2','--max-skew','3']);
assert.equal(duplicateOption.ok,false);
assert.equal(duplicateOption.reason,'DUPLICATE_MAX_SKEW_OPTION');
const unknownOption=parseDualFeedCliArgs(['first.har','--unsafe']);
assert.equal(unknownOption.ok,false);
assert.equal(unknownOption.reason,'UNKNOWN_OPTION');
...
assert.equal(result.version,'betfair-sporting-dual-feed-cli-v1.3-prospective-status');
...
assert.equal(result.prospective.valid,false);
assert.equal(result.prospective.reason,'CAPTURE_NOT_STRICTLY_AFTER_FROZEN_PROTOCOL_COMMIT');
assert.equal(result.prospective.prospectiveCalibrationCandidate,false);
...
assert.equal(result.hardGuards.cliFirstInputNeverDroppedWhenOptionAbsent,true);
assert.equal(result.hardGuards.prospectiveStatusPinnedToFrozenProtocolCommit,true);
...
const future=analyzeSafeDualFeedTexts([
  {raw:raw(1787773200,223.45,1787774000),sourceName:'future-one.har'},
  {raw:raw(1787773210,223.55,1787774000),sourceName:'future-two.har'},
  {raw:raw(1787773220,223.65,1787774000),sourceName:'future-three.har'},
],{maxCaptureSkewSeconds:2});
assert.equal(future.ok,true);
assert.equal(future.series.empiricalModernResponseMappingVerified,true);
assert.equal(future.prospective.valid,true);
assert.equal(future.prospective.freezeCommitSha,'3f397f820914bfdd39b42e4bd5262bd1b986751f');
assert.equal(future.prospective.allCapturesStrictlyAfterFreezeCommit,true);
assert.equal(future.prospective.strictCaptureOrderVerified,true);
assert.equal(future.prospective.prospectiveCalibrationCandidate,true);
assert.equal(future.prospective.completeAttemptLedgerVerified,false);
assert.equal(future.prospective.exactModernResponseSemanticsVerified,false);
assert.equal(future.prospective.usableForOverduePair,false);
assert.equal(future.prospective.execution.realMoneyAllowed,false);
