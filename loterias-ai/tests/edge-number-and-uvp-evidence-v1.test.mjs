...
assert.equal(uvp.economic.paytableVerified,true);
assert.equal(uvp.economic.visiblePaytable.fullHouse,7);
assert.equal(uvp.economic.visiblePaytable.flush,5);
assert.equal(uvp.economic.visiblePaytable.ordinaryRoyalFlush,800);
assert.equal(uvp.economic.qualifyingStakeVerified,false);
assert.equal(uvp.execution.exactStakeKnown,true);
assert.equal(uvp.execution.stakePerDecisionEUR,2.5);
