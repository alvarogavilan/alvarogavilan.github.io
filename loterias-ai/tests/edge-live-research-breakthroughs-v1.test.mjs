...
assert.equal(norseCard.closedGates,5);
...
assert.equal(norseCard.identityClosed,1);
assert.equal(norseCard.identityTotal,2);
assert.equal(norseCard.liveClosed,0);
assert.equal(norseCard.liveTotal,3);
assert.equal(norseCard.dailyConfiguredDeploymentVerified,true);
assert.equal(norseCard.providerDailyIdentityVerified,true);
...
assert.match(norseHtml,/IDENTIDAD/);
assert.match(norseHtml,/1\/2/);
assert.match(norseHtml,/ESTADO LIVE/);
assert.match(norseHtml,/0\/3/);
