...
const rule='Frank Bruno Sporting Legends jackpot rules. If no gameplay takes place when the Daily Jackpot must be won, the jackpot will be triggered by the first bet placed the following day. Any bet of any size can win the Daily Jackpot, but a larger bet has a greater chance. The jackpots are funded by the operator and do not affect the RTP of the game.';
...
assert.equal(r.reason,'RULE_ELIGIBILITY_AND_RTP_SEPARATION_REVIEW_CANDIDATES_ASSEMBLED');
...
assert.equal(r.jackpotRtpSeparationReviewCandidate,true);
assert.equal(r.allThreeSemanticsReviewCandidatesPresent,true);
...
assert.equal(r.headlineRtpMayBeUsedAsBaseGameRtp,false);
...
assert.equal(r.ruleEvidence.operatorFundedRtpSeparationBodySha256.length,1);
...
assert.equal(r.valid,true);assert.equal(r.reason,'NO_EXECUTION_OR_ECONOMIC_SEMANTICS_REVIEW_CANDIDATE_FOUND');assert.equal(r.followingDayOperatorRuleReviewCandidate,false);assert.equal(r.tenCentJackpotEligibilityReviewCandidate,false);assert.equal(r.jackpotRtpSeparationReviewCandidate,false);
...
assert.equal(r.valid,true);assert.equal(r.reason,'PARTIAL_EXECUTION_OR_ECONOMIC_SEMANTICS_REVIEW_CANDIDATES_ASSEMBLED');assert.equal(r.followingDayOperatorRuleReviewCandidate,false);assert.equal(r.tenCentJackpotEligibilityReviewCandidate,true);assert.equal(r.jackpotRtpSeparationReviewCandidate,false);

r=build(har({ruleText:'Frank Bruno Sporting Legends jackpot rules. The jackpots are funded by the operator and do not affect the RTP of the game.'}));
assert.equal(r.valid,true);assert.equal(r.reason,'PARTIAL_EXECUTION_OR_ECONOMIC_SEMANTICS_REVIEW_CANDIDATES_ASSEMBLED');assert.equal(r.followingDayOperatorRuleReviewCandidate,false);assert.equal(r.tenCentJackpotEligibilityReviewCandidate,false);assert.equal(r.jackpotRtpSeparationReviewCandidate,true);
