...
assert.equal(s.live.total,5);
assert.equal(s.open.length,5);

const progressed={p0Strategy:{stateObservationGate:{...data.p0Strategy.stateObservationGate,directGameToAognjp2BindingVerified:true,sameSessionDailyActiveVerified:true,currentDailyJackpotEUR:123.45,guaranteedHitTime:1780000000,exactTickerHostRecovered:true,exactImsCasinoRecovered:true},currentDeploymentConfigurationGate:data.p0Strategy.currentDeploymentConfigurationGate,tickerIdentityGate:{}}};
const p=summarizeP0NorseProgress(progressed);
assert.equal(p.closed,9);
assert.equal(p.pct,100);
assert.equal(p.live.closed,5);
