...
function buildOnlineCards(data){
  ...
  return data.candidates.map((c)=>{
    ...
    return {
      ...
      crossPlayerVerified:crossPlayer,preWagerVisibleVerified:preWager,exactConfigVerified:exactConfig,status:'P0 · INVESTIGACIÓN',action:'NO_PLAY',sourceType:'ONLINE',
...
function buildPhysicalCards(data){
  ...
  return data.candidates.map((c)=>{
    ...
    return {
      ...
      guardText:deterministic?'La fórmula está demostrada. El siguiente cierre no requiere apostar: capturar en la máquina exacta la placa/autorización (modelo Galicia y serie), la pantalla frontal/help (apuestas, denominaciones, paytable y retorno mostrado) y el ciclo/persistencia antes de cualquier promoción.':scarab?'Falta confirmar sin apostar la configuración local, paytable y estado abandonado antes de transferir la regla de entrada.':'Falta resolver el nombre canónico y fingerprint local completo antes de transferir cualquier estrategia Ocean Magic.',
      decisiveBlocker:'FALTA_FINGERPRINT_LOCAL_SIN_APOSTAR'
    };
  });
}

export function buildBreakthroughCards(data){return data?.venue?buildPhysicalCards(data):buildOnlineCards(data);}
export function buildCombinedBreakthroughCards(datasets){return (Array.isArray(datasets)?datasets:[]).flatMap(buildBreakthroughCards);}
