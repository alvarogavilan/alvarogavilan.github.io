...
function render(){
  ...
  $('instruction').textContent=ready?`CONFIRMA VERDE. Apuesta ${eur(p.order.stakePerSpinEUR)} por giro, máximo ${p.order.maxSpins} giros. Si pasa a rojo, PARA.`:prepare?'ABRE BOTEMANIA, entra en Fishin’ Frenzy y déjalo listo. NO hagas ningún giro hasta recibir JUGAR AHORA.':'NO JUGAR. No realices ningún giro.';
...
$('playButton').addEventListener('click',()=>{if(phase()!=='RED')window.open(GAME_URL,'_blank','noopener');});setInterval(()=>{$('clock').textContent=clock();render();},1000);setInterval(refreshScientific,5000);setInterval(directProbe,3000);$('clock').textContent=clock();refreshScientific();directProbe();
