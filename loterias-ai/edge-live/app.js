...
function whyText(){const blockers=Array.isArray(state.plan?.blockers)?state.plan.blockers:[];if(planReady())return 'Todos los gates de ejecución están vigentes. Sigue exactamente la apuesta por giro, máximo de giros y caducidad indicados arriba.';if(!blockers.length)return 'No hay una orden ejecutable vigente. EDGE LIVE se mantiene cerrado por seguridad.';return blockers.map(b=>`• ${blockerLabel[b]||escapeHtml(b)}`).join('<br>');}
function renderArtwork(){
  ...
  const url=graphql||probed;
  if(!url)return;
  $('gameArt').textContent='';$('gameArt').style.backgroundImage=`url("${url}")`;$('gameBg').style.backgroundImage=`url("${url}")`;
}
function render(){
  const p=state.plan||{},l=state.live||{},v=state.validation||{},ready=planReady(),pots=visiblePots();
  $('potKing').textContent=eur(pots.JACKPOT_KING);$('potRegal').textContent=eur(pots.REGAL);$('potRoyal').textContent=eur(pots.ROYAL);$('gameCard').href=GAME_URL;$('inspectGame').href=GAME_URL;renderArtwork();
  $('orderCard').className='order'+(ready?' ready':'');$('decision').textContent=ready?'JUGAR AHORA':'NO JUGAR';$('signalState').textContent=ready?'SEÑAL VÁLIDA':'BLOQUEADO';
  $('stakePerSpin').textContent=ready?eur(p.order.stakePerSpinEUR):'—';$('maxSpins').textContent=ready?String(p.order.maxSpins):'0';$('maxTotal').textContent=ready?eur(p.order.maxTotalStakeEUR):'0,00 €';$('entryWindow').textContent=ready?`AHORA · hasta ${time(p.order.validUntil)}`:'NO JUGAR';
  const remaining=ready?Math.max(0,Math.ceil((Date.parse(p.order.validUntil)-Date.now())/1000)):null;$('expiry').textContent=ready?`${remaining}s`:'—';$('instruction').textContent=ready?`Abre el juego real. Apuesta ${eur(p.order.stakePerSpinEUR)} por giro, máximo ${p.order.maxSpins} giros, y termina antes de ${time(p.order.validUntil)}.`:'No hay una instrucción económica completa y vigente. No realices ninguna jugada.';
  ...
  const age=secAge(l?.current?.observedAt);$('freshness').textContent=age==null?'SIN DATO':`${age}s`;$('freshness').className=age!=null&&age<=90?'ok':age!=null&&age<=360?'warn':'bad';$('rtp').textContent=pct(l?.current?.modelScreen?.bestConservativeRtp);$('observed').textContent=time(l?.current?.observedAt);$('channel').textContent=directFresh()?'DIRECTO':'EVIDENCIA';$('why').innerHTML=whyText();$('liveDot').style.background=ready?'var(--green)':(age!=null&&age<=90?'var(--amber)':'var(--red)');
}
...
$('playButton').addEventListener('click',()=>{if(planReady())window.open(GAME_URL,'_blank','noopener');});setInterval(()=>{$('clock').textContent=clock();render();},1000);setInterval(refreshScientific,5000);setInterval(directProbe,3000);$('clock').textContent=clock();refreshScientific();directProbe();
