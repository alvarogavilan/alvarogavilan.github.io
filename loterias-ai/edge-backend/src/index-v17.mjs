...
import { summarizeDurations,elapsedPercentile } from './jackpot-timing-core-v1.mjs';
...
function localParts(ms){const parts=dtf.formatToParts(new Date(ms));const out={hour:null,weekday:null};for(const p of parts){if(p.type==='hour')out.hour=Number(p.value);if(p.type==='weekday')out.weekday=p.value;}return out;}
...
export class EdgeSentinel extends V16EdgeSentinel{
  timingRows(){
    ...
    for(const meterKey of meterKeys){
      ...
      const intervalStats=summarizeDurations(durations),cv=intervalStats.coefficientOfVariation,n=intervalStats.n;
      ...
      const agePct=elapsedPercentile(durations,elapsed);
  ...
  timingResearch(meterKey=null){
    ...
    return {
      ...
      guards:{candidateBoundaryIsNotVerifiedAward:true,elapsedPercentileIsNotDueProbability:true,clockClusteringWithoutExposureIsNotHazard:true,rngHistoryDoesNotPredictNextIndependentOutcome:true,explicitTimedMechanismRequiresRuleVerification:true,prospectiveValidationRequiredBeforeTimingClaim:true,timingResearchCannotEnableRealMoney:true,executionContractRemainsSoleGreenAuthority:true,realMoneyAllowed:false}
    };
  }
  ...
  async fetch(request){
    ...
    if(path==='/science/timing'){
      await this.ensureAlarm();try{await this.updateCycleLedger();}catch{}
