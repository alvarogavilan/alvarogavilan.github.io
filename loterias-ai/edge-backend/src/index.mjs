...
export class EdgeSentinel {
  ...
  persistSnapshots(nowMs, observedAt, meters) {
    ...
    this.sql.exec(`INSERT OR REPLACE INTO minute_snapshots(minute_epoch, observed_at, meters_json) VALUES (?, ?, ?)`, minuteEpoch, observedAt, metersJson);
    this.sql.exec(`INSERT OR REPLACE INTO hourly_snapshots(hour_epoch, observed_at, meters_json) VALUES (?, ?, ?)`, hourEpoch, observedAt, metersJson);
    if (new Date(nowMs).getUTCMinutes() === 0 && new Date(nowMs).getUTCSeconds() < 10) {
  ...
  persistMeterEvents(previousMeters, meters, observedAt, nowMs) {
    for (const [key, amountEUR] of Object.entries(meters)) {
      ...
      const type = delta < -0.01 ? 'DROP_CANDIDATE' : 'METER_CHANGE';
      if (type === 'METER_CHANGE' && Math.abs(delta) < 0.01) continue;
      this.sql.exec(
  ...
  async alarm() {
    ...
    try {
      ...
      this.persistSnapshots(nowMs, observedAt, feed.canonical);
      ...
      const state = {
        serviceVersion: 'edge-sentinel-v1',
        observedAt,
        observedAtMs: nowMs,
