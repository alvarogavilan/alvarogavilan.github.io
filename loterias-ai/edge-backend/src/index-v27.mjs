...
export const DEPLOYMENT_FINGERPRINT='edge-sentinel-v27-playuzu-cgm-fast-profit-20260824a';
...
export class EdgeSentinel extends V26EdgeSentinel{
  ...
  fastProfitResearch(){
    ...
    return {
      ...base,version:'edge-fast-profit-lab-v4-playuzu-cgm',generatedAt:new Date().toISOString(),
      lanes:[
      ...
      guards:{...base.guards,zeroCapitalClaimCannotBeCalledGuaranteedProfit:true,positiveEvSignNeedsPositiveCashoutProbability:true,playuzuExternalRtpCannotPromote:true,realMoneyAllowed:false}
    };
  }
  ...
  async fetch(request){
    ...
    if(path==='/science/playuzu-welcome'){
      ...
      return responseJson({ok:true,service:'loterias-edge-sentinel',deploymentFingerprint:DEPLOYMENT_FINGERPRINT,deploymentCapabilities:{playuzuWelcomeScreenV27:true,oneSpinBreakEvenEnvelope:true,accountPolicyConflictGuard:true,executionContractFailClosed:true},research:this.playuzuWelcomeResearch()});
    }
    if(path==='/science/cgm-zero-deposit'){
      await this.ensureAlarm();
      return responseJson({ok:true,service:'loterias-edge-sentinel',deploymentFingerprint:DEPLOYMENT_FINGERPRINT,deploymentCapabilities:{cgmZeroDepositScreenV27:true,zeroCapitalDownsideModel:true,bonusSurvivalGate:true,executionContractFailClosed:true},research:this.cgmZeroDepositResearch()});
    }
    const response=await super.fetch(request);
    if(!['/','/health','/state','/science/status','/science/events','/science/jpk','/science/winfall','/science/ath','/science/cycles','/science/timing','/science/history-depth','/science/spanish-awards','/science/jokerbet-stack','/science/jokerbet-promos','/science/fast-profit','/science/paf-group-promos','/library/summary','/library/sources','/library/search','/library/record','/library/coverage','/library/universe'].includes(path))return response;
    try{
      const body=await response.clone().json();
      body.deploymentFingerprint=DEPLOYMENT_FINGERPRINT;
      body.deploymentCapabilities={...(body.deploymentCapabilities||{}),playuzuWelcomeScreenV27:true,cgmZeroDepositScreenV27:true,zeroCapitalDownsideModel:true,oneSpinBreakEvenEnvelope:true};
